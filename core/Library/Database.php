...
class Database 
{
    ...
    */
    ...
    {
        $aRegistro = $rscRes->fetch(PDO::FETCH_ASSOC);
        return $aRegistro[$CampoRetorno];
    }
...
{
    try {
        // 🔥 SEGURANÇA: NÃO DEIXA UPDATE SEM WHERE
        if (empty($this->where)) {
            throw new \Exception("🚨 UPDATE sem WHERE bloqueado por segurança.");
        }

        if (empty($this->table)) {
            throw new \Exception("🚨 Tabela não definida no update.");
        }
        ...
        Session::set("msgError", $err->getMessage());
        $rs = 0;
    }
