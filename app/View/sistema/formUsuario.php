...
<form method="POST" action="<?= $this->request->formAction() ?>">
    ...
    <div class="row m-2">
        ...
        <!-- Status (somente administradores) -->
        <?php if (in_array(\Core\Library\Session::get("userNivel"), [1, 11])): ?>
        ...
        <?php else: ?>
            <!-- Usuário comum: valor fixo -->
            <input type="hidden" name="statusRegistro" value="<?= setValor('statusRegistro') ?: 1 ?>">
        <?php endif; ?>

        <!-- Nível (somente administradores) -->
        <?php if (in_array(\Core\Library\Session::get("userNivel"), [1, 11])): ?>
            <div class="mb-3 col-4">
        ...
        <?php else: ?>
            <!-- Usuário comum: nível fixo -->
            <input type="hidden" name="nivel" value="21">
        <?php endif; ?>
        ...
        <!-- Senha (exibida para insert ou update) -->
