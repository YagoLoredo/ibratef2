    ...
    <?php endif; ?> 
        ...
        <div class="row">
            ...
            <option value="" <?= setValor('cliente_id') == "" ? "SELECTED" : "" ?>>...</option>
                <?php foreach ($dados['aCliente'] as $value): ?>
                <option value="<?= $value['id'] ?>" <?= setValor('cliente_id') == $value['id'] ? "SELECTED" : "" ?>>
                <?= $value['nome'] ?>
            </option>
                <?php endforeach; ?>
             </select>
            ...
            <div class="col-md-3 mb-3">
                ...
                <input type="date" class="form-control"
                       name="vencimento"
                       id="vencimento"
                       value="<?= setValor("vencimento") ?>"
                       required>
                <?= setMsgFilderError("vencimento") ?>
            </div>
            ...
            <?php if (in_array($this->request->getAction(), ['insert', 'update'])): ?>
                <label for="arquivo" class="form-label">Arquivo do Boleto (PDF)</label>
                <input type="file"
                       class="form-control"
                       name="arquivo"
                       id="arquivo"
                ...
                <?php if (!empty($data['arquivo'])): ?>
                    <div class="mt-2">
                        <a href="/boleto/download/<?= $data['id'] ?>" class="btn btn-sm btn-outline-primary botoes">
                            📄 Ver PDF atual
                        </a>
                    </div>
                <?php endif; ?>
