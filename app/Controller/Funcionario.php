    ...
    public function update()
{
    ...
    // 1. Buscar os dados atuais do funcionário para obter o usuario_id
    ...
    if (!$lError) {
        // 2. Tenta atualizar o Funcionário primeiro
        if ($this->model->update($post)) {
            
            // ==========================================
            // 3. SINCRONIZAR COM A TABELA DE USUÁRIOS
            // ==========================================
            $usuarioModel = new \App\Model\UsuarioModel();
            
            $dadosUsuario = [
                "id"    => $data["usuario_id"], // ID da tabela usuario
                "nome"  => trim($post['nome']),
                "email" => trim($post['email'])
            ];

            // Atualiza o nome e email na tabela de login
            $usuarioModel->update($dadosUsuario);
            // ==========================================

            return Redirect::page($this->controller, ["msgSucesso" => "Funcionário e Usuário atualizados com sucesso."]);
        } else {
            $lError = true;
        }
    }
